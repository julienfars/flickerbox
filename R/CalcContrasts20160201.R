source("FtoBinary.R")
source("ContrastFunctions.R")
source("WritePresetFile.R")

load("ConeFundamentalsVlambda.rda")
load("LED.rda")
load("ConeFundamentalsStockman10deg.rda")

## calculate conversion luminance to energy
# calculate conversion factors from luminance to energy
lum <- crossprod(as.matrix(LED1[, 2:5]),
                 as.matrix(CF_vlambda$Vlambda))
